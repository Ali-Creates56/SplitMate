...
import { User, Group, SuggestedSettlement } from "../types";
import { 
  TrendingUp, 
  ArrowDownLeft, 
  ArrowUpRight, 
    ...
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center relative">
        <div className="flex items-center gap-3">
          <div 
            ...
            className="w-11 h-11 rounded-full overflow-hidden border-2 border-emerald-500 shadow-sm cursor-pointer hover:scale-105 transition-transform"
          >
            <img 
          ...
          <div>
            <div className="flex items-center gap-1">
              <p className="text-xs text-slate-500 dark:text-slate-400">Wellcome Back</p>
            </div>
        ...
        {/* Dropdown to emulate other users easily */}
        {showUserMenu && (
          <div className="absolute top-14 left-0 w-64 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-2xl p-3 z-50 animate-in fade-in slide-in-from-top-2">
            <p className="text-[10px] uppercase tracking-wider font-bold text-slate-400 mb-2 px-2">Emulate Another Account:</p>
      ...
      <div id="net-balance-card" style={{ backgroundColor: '#000000' }} className="glass dark:dark-glass rounded-3xl p-6 shadow-lg shadow-emerald-950/5 relative overflow-hidden">
        ...
        <div className="flex flex-col sm:flex-row gap-4">
          ...
          <div className="hidden sm:block w-px h-8 bg-slate-800 self-center" />
          <div className="flex-1 flex items-center gap-3">
            <div className="w-8 h-8 rounded-full bg-rose-500/10 dark:bg-rose-400/10 flex items-center justify-center text-rose-600 dark:text-rose-400 shrink-0">
              <ArrowUpRight className="w-4 h-4" />
            </div>
            <div>
              <p className="text-xs text-slate-400 font-medium whitespace-nowrap">To give</p>
              <p className="text-sm font-bold text-white mt-0.5 font-mono">
                {formatCurrency(conv(stats.amountYouOwe), currency)}
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="space-y-3">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          <button 
            ...
            className="p-4 bg-emerald-600 hover:bg-emerald-700 text-white rounded-2xl flex flex-col items-center justify-center gap-2 transform active:scale-95 transition-all text-center group"
          >
            <PlusCircle className="w-6 h-6 group-hover:rotate-90 transition-transform" />
            <span className="text-xs font-semibold tracking-wide">Add Expense</span>
          </button>
          ...
          >
            ...
            <span className="text-xs font-semibold tracking-wide">View Reports</span>
          </button>
        </div>
      </div>
