...
export default function Analytics({ groups, onClose, currency }: AnalyticsProps) {
  // Aggregate total expenses across all active logged in groups
  let totalSpendSum = 0;
  const categoryMap: Record<string, number> = {};
  const groupContributions: { name: string; amount: number; percentage: number }[] = [];
  ...
  const categoriesList = [
    { name: "Housing & Stays", amount: Math.round(totalSpendSum * 0.55), percentage: 55, color: "bg-emerald-500" },
    { name: "Food & Dining", amount: Math.round(totalSpendSum * 0.25), percentage: 25, color: "bg-amber-500" },
    { name: "Transport & Uber", amount: Math.round(totalSpendSum * 0.12), percentage: 12, color: "bg-sky-500" },
    { name: "General Miscellaneous", amount: Math.round(totalSpendSum * 0.08), percentage: 8, color: "bg-slate-500" }
  ];
